...
$sql = "SELECT unique_id FROM Persons WHERE position LIKE CONCAT('%', '3', '%')";
...
foreach ($data as $u) {
    ...
    $sql = "SELECT * FROM Requests WHERE is_closed=3 AND req_status=3 AND unit=3 ORDER BY accept_date DESC, accept_time DESC LIMIT 1000";
    if ($result = $conn->query($sql)) {
        if ($result->num_rows == 0) {
